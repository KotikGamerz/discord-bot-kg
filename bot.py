...
HNYC_MORNING_TIPS = [
    "☀️ Сделай первый глоток воды или чая и включи тихую музыку — пусть день начнётся мягко и красиво. 🎶",
    ...
    "🌫️ Сделай тихий старт: пять минут без суеты — и дальше день пойдёт заметно плавнее. ✨",
    "🕯️ Я здесь, как новогодний огонёк на фоне — давай начнём день спокойно и уверенно, шаг за шагом. 🎄",
    "🌟 Праздник уже рядом, но и этот день важен — выбери один приятный момент и проживи его полностью."
]
...
GMT2 = timezone(timedelta(hours=2))  # твой ориентир

def now_gmt2():
    return datetime.datetime.now(GMT2)

# Расписание: час (GMT+2) -> текст стран/территорий.
# ⚠️ Это НЕ «все страны мира по паспорту», потому что многие страны живут в 2-11 таймзонах.
# Зато это стабильная, понятная “таймзонная” логика, как у большинства NY ботов.
HNYC2_SCHEDULE = {
    "12:00": "🇰🇮 Кирибати (Острова Лайн, UTC+14)",
    "13:00": "🇳🇿 Новая Зеландия (основная зона), 🇫🇯 Фиджи, 🇹🇻 Тувалу, 🇲🇭 Маршалловы Острова (UTC+12)",
    "14:00": "🇸🇧 Соломоновы Острова, 🇻🇺 Вануату, 🇳🇨 Новая Каледония (UTC+11)",
    "15:00": "🇦🇺 Австралия (восток), 🇵🇬 Папуа–Новая Гвинея (UTC+10)",
    "16:00": "🇯🇵 Япония, 🇰🇷 Южная Корея (UTC+9)",
    "17:00": "🇨🇳 Китай, 🇵🇭 Филиппины, 🇸🇬 Сингапур, 🇲🇾 Малайзия, 🇭🇰 Гонконг (UTC+8)",
    "18:00": "🇹🇭 Таиланд, 🇻🇳 Вьетнам, 🇰🇭 Камбоджа, 🇱🇦 Лаос (UTC+7)",
    "19:00": "🇧🇩 Бангладеш, 🇧🇹 Бутан (UTC+6)",
    "20:00": "🇮🇳 Индия, 🇱🇰 Шри-Ланка (UTC+5:30 / около UTC+5)",
    "21:00": "🇦🇪 ОАЭ, 🇴🇲 Оман (UTC+4)",
    "22:00": "🇷🇺 Россия (Москва), 🇧🇾 Беларусь, 🇹🇷 Турция (UTC+3)",
    "23:00": "🇲🇩 Молдова, 🇷🇴 Румыния, 🇺🇦 Украина, 🇬🇷 Греция (UTC+2)",
    "00:00": "🇩🇪 Германия, 🇫🇷 Франция, 🇪🇸 Испания (основная зона), 🇮🇹 Италия (UTC+1)",
    "01:00": "🇬🇧 Великобритания, 🇵🇹 Португалия (UTC+0)",
    "02:00": "🇨🇻 Кабо-Верде (UTC-1)",
    "03:00": "🇧🇷 Бразилия (восток), 🇦🇷 Аргентина, 🇺🇾 Уругвай (UTC-3)",
    "04:00": "🇨🇱 Чили (часть), 🇧🇴 Боливия (UTC-4)",
    "05:00": "🇺🇸 США (восток), 🇨🇦 Канада (восток) (UTC-5)",
    "06:00": "🇺🇸 США (центр), 🇨🇦 Канада (центр) (UTC-6)",
    "07:00": "🇺🇸 США (гора), 🇨🇦 Канада (гора) (UTC-7)",
    "08:00": "🇺🇸 США (тихоокеанское), 🇨🇦 Канада (тихоокеанское) (UTC-8)",
    "09:00": "🇺🇸 США (Аляска) (UTC-9)",
    "10:00": "🇵🇫 Французская Полинезия (часть), 🇺🇸 Гавайи (UTC-10)",
    "11:00": "🇦🇸 Американское Самоа (UTC-11)",
...
async def _safe_send(channel: disnake.TextChannel, text: str) -> bool:
    """Отправка с мягкой защитой: если словим 429/ошибку — просто вернём False."""
    try:
        await channel.send(text)
        return True
    except disnake.HTTPException as e:
        # 429 и прочие HTTP ошибки
        print(f"⚠️ HNYC2 send failed: {getattr(e, 'status', 'HTTP')} {e}")
        return False
...
async def hnyc2_loop():
    ...
    if now > end + timedelta(hours=2):
        # слишком поздно — считаем процесс устаревшим и выключаем
        cfg["enabled"] = False
        cfg["finished"] = True
    ...
    # Чтобы не убить лимиты, за 1 тик догоняем максимум 2 сообщения
